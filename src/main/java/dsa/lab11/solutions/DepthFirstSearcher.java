...
public class DepthFirstSearcher
{
    ...
    DynamicSequence<Vertex> visited)
  {
    ...
    for (Vertex neighbour : graph.neighbours(vertex))
    {
      ...
      {
        // NOTE: If the neighbour is new (i.e. we haven't already found a path
        //       to it), note that we can get to it from this vertex and visit
        //       it (i.e. visit its neighbours too).
        paths.insert(neighbour, vertex);
        visit(graph, neighbour, paths, visited);
      }
    }

    // NOTE: Add this vertex to the list of fully-visited vertices.
