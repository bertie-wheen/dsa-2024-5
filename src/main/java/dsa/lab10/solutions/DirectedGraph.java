  ...
  implements Container<Vertex>
{
  ...
  {
    // NOTE: We first check whether the edge _might_ be an edge in this graph,
    //       i.e. whether its vertices are in the graph, before checking whether
    //       it actually is or not by looking up its source's outgoing edges and
    //       seeing if it's among them.
    return this.contains(edge.source()) && this.contains(edge.target()) &&
      this.adjacencyLists.get(edge.source()).contains(edge);
  }
  ...
  public Edge<Vertex, Weight> edge(Vertex source, Vertex target)
  {
    ...
    // NOTE: If there is an edge between the given source and target vertices,
    //       it will be amongst the outgoing edges from the source vertex.
    // NOTE: We therefore get the relevant adjacency list and check each edge to
    //       see which vertex is it going to, i.e. whether it is going to the
    //       given target. If it is, then we return that edge.
    for (Edge<Vertex, Weight> edge : this.adjacencyLists.get(source))
    {
    ...
    // NOTE: If there was such an edge, we would have found it in the previous
    //       for-each loop, so if we get to this point then there must be no
    //       edge from the given source to the given target, so we return null.
    return null;
  }
